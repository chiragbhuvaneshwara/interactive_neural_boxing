...
import shutil
from ztrain_and_server.src.nn.mann_keras_v2.mann import MANN, loss_func, loss_func_2, prepare_mann_data, \
...
if __name__ == '__main__':
    ...
    FRD = 1
    WINDOW = 15
    OUT_BASE_PATH = os.path.join("ztrain_and_server", "saved_models", "mann_tf2_v2")
    ############################################
    frd_win = 'boxing_fr_' + str(FRD) + '_' + str(WINDOW)
    current_timestamp = datetime.now().strftime("%Y%m%d_%H-%M-%S")

    if not DEVELOP:
        dataset_config_path = os.path.join("ztrain_and_server", "data", frd_win, "config.json")
        dataset_npz_path = os.path.join("ztrain_and_server", 'data', frd_win, 'train.npz')
        batch_size = 32

    elif DEVELOP:
        print('Dev Mode')
        OUT_BASE_PATH = os.path.join(OUT_BASE_PATH, "dev")
        dataset_config_path = os.path.join("ztrain_and_server", "data", "dev", frd_win, "config.json")
        dataset_npz_path = os.path.join("ztrain_and_server", 'data', 'dev', frd_win, 'train.npz')
        batch_size = 2
        EPOCHS = 2
