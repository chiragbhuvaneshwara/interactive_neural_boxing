from .layers.layers_tf import TF_PFNN_Layer, TF_Variational_Layer
...
class VINN(PFNN):
	...
	def __init__(self, input_size, output_size, hidden_size, norm, batch_size = 32, layers = [], dropout = 0.7, replace_layers = [], draw_each_layer = False, random_number_dim = -1):
		...
		if len(layers) != 3:
			# l0 = TF_Variational_Layer((hidden_size, input_size), elu_operator = tf.nn.elu, name="Layer0")
			...
			# if 0 in replace_layers:
			# 	l0 = TF_Variational_Layer(l0)
			# if 1 in replace_layers:
			# 	l1 = TF_Variational_Layer(l1)
			# if 2 in replace_layers:
			# 	l2 = TF_Variational_Layer(l2)
	...
	def load(target_file):
		with open(target_file, "r") as f:
			...
			replace_layers = []
			if "VINN" in store["layer_0"]:
				l0 = TF_Variational_Layer.load([store["layer_0"], tf.nn.elu])
				replace_layers.append(0)
			else:
				l0 = TF_PFNN_Layer.load([store["layer_0"], tf.nn.elu])
			if "VINN" in store["layer_1"]:
				l1 = TF_Variational_Layer.load([store["layer_1"], tf.nn.elu])
				replace_layers.append(1)
			else:
				l1 = TF_PFNN_Layer.load([store["layer_1"], tf.nn.elu])
			if "VINN" in store["layer_2"]:
				l2 = TF_Variational_Layer.load([store["layer_2"], None])
				replace_layers.append(2)
			else:
				l2 = TF_PFNN_Layer.load([store["layer_2"], None])
	...
	def train(self, X, Y, epochs, target_path):
		...
		"""
		super().train(X, Y, epochs, target_path)
		# self.build_tf_graph([])
	...
	def forward_pass(self, params):
		...
		# if self.draw_each_layer:
		# 	for l in self.layers:
		# 		z = tf.random.normal((1, self.random_number_dim), 0.0, 1.0, dtype = tf.float32)
		# 		l.set_random_state(z)
		# else:
		# 	z = tf.random.normal((1, self.random_number_dim), 0.0, 1.0, dtype = tf.float32)
		# 	for l in self.layers:
		# 		l.set_random_state(z)
	...
	def from_file(dataset, target_path, epochs, config_store, replace_layers = [], draw_each_layer = False, random_number_dim = -1):
		...
		Ystd[2:3] = Ystd[2:3].mean()  # Rotational Velocity / Dir x
		Ystd[3:4] = Ystd[3:4].mean()  # Change in Phase  / Dir y
		Ystd[4:5] = Ystd[4:5].mean() # Change in Phase
		start = 5
		...
		vinn = VINN(input_dim, output_dim, 256, norm, replace_layers = replace_layers, draw_each_layer = draw_each_layer, random_number_dim = random_number_dim)

		vinn.train(X, Y, epochs, target_path)