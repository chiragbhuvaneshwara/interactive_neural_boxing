...
{
  public partial class T_MultiThreadedMosi {
    public interface ISync {
      int registerSession();
      TPosture getZeroPosture();
      TPosture fetchFrame(double time, TPosture currentPosture, TVector3 direction, TGait gait, int session);
    }
    ...
    public interface Iface : ISync {
      ...
      #endif
      #if SILVERLIGHT
      IAsyncResult Begin_fetchFrame(AsyncCallback callback, object state, double time, TPosture currentPosture, TVector3 direction, TGait gait, int session);
    ...
    public class Client : IDisposable, Iface {
      ...
      public IAsyncResult Begin_fetchFrame(AsyncCallback callback, object state, double time, TPosture currentPosture, TVector3 direction, TGait gait, int session)
      {
        return send_fetchFrame(callback, state, time, currentPosture, direction, gait, session);
      }
      ...
      public TPosture fetchFrame(double time, TPosture currentPosture, TVector3 direction, TGait gait, int session)
      {
        #if SILVERLIGHT
        var asyncResult = Begin_fetchFrame(null, null, time, currentPosture, direction, gait, session);
        return End_fetchFrame(asyncResult);

        #else
        send_fetchFrame(time, currentPosture, direction, gait, session);
        return recv_fetchFrame();

        #endif
      }
      #if SILVERLIGHT
      public IAsyncResult send_fetchFrame(AsyncCallback callback, object state, double time, TPosture currentPosture, TVector3 direction, TGait gait, int session)
      {
      ...
      public void send_fetchFrame(double time, TPosture currentPosture, TVector3 direction, TGait gait, int session)
      {
        oprot_.WriteMessageBegin(new TMessage("fetchFrame", TMessageType.Call, seqid_));
        fetchFrame_args args = new fetchFrame_args();
        args.Time = time;
    ...
    public class Processor : TProcessor {
      ...
      public void fetchFrame_Process(int seqid, TProtocol iprot, TProtocol oprot)
      {
        ...
        try
        {
          result.Success = iface_.fetchFrame(args.Time, args.CurrentPosture, args.Direction, args.Gait, args.Session);
